...
#'
#' `ggplot` graphics are quite different from the base graphics package you might be used to. With `ggplot` you build up a figure piece by piece, with different functions for each element of the figure. This can lead to quite a lot of code required for a single figure. However, the payoff is a much more highly customisable figure that can be made to fit publication guidelines.
#'
#' The code below creates a png file to store the figure, uses `ggplot` to customise the elements of the figure and uses a colour palette from the `RColorBrewer` package.
#+ eval = FALSE
...
#' 
#' 
#' *Note on this handbook - This handbook is written in R, translated into Markdown via `knitr::spin` and then compiled to html. Both .R and .Rmd files are available in the Github repository (LINK) to demonstrate how the handbook was produced.*
#' 
